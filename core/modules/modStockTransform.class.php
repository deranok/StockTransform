...
class modStockTransform extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->numero = 555445; // TODO Go on page https://wiki.dolibarr.org/index.php/List_of_modules_id to reserve an id number for your module

		// Key text used to identify module (for permissions, menus, etc...)
		//~ $this->rights_class = 'stocktransform';
    ...
    $this->name = $this->name = preg_replace('/^mod/i', '', get_class($this));
    
		// DESCRIPTION_FLAG
		// Module description, used if translation string 'ModuleMyModuleDesc' not found (MyModule is name of module).
		$this->description = "Easily transform stock from one or more products to another";
		// Used only if file README.md and README-LL.md not found.
		$this->descriptionlong = $this->description;

		// Author
		$this->editor_name = 'Oppey Jeff';
		$this->editor_url = 'https://github.com/deranok';		// Must be an external online web site
		...
		$this->hidden = getDolGlobalInt('MODULE_STOCKTRANSFORM_DISABLED'); // A condition to disable module;
		...
		//~ $this->langfiles = array("setup.php@stocktransform");
		...
		if (!isModEnabled("stocktransform")) {
			$conf->stocktransform = new stdClass();
			$conf->stocktransform->enabled = 0;
		}
    ...
    /*
		$this->menu[$r++] = array(
			...
			'position'=>1000 + $r,
			'enabled'=>'isModEnabled("stocktransform")', // Define condition to show or hide menu entry. Use 'isModEnabled("mymodule")' if entry must be visible if module is enabled.
    ...
    */
		...
		$this->menu[$r++]=array(
			...
			'titre'=>'Stock Transform',
			'prefix' => img_picto('', $this->picto, 'class="pictofixedwidth valignmiddle paddingright"'),
			'mainmenu'=>'products',
			'leftmenu'=>'',
			'url'=>'/StockTransform/htdocs/index.php',
			//~ 'langs'=>'mymodule@mymodule',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
			'position'=>1000+$r,
			'enabled'=>'isModEnabled("stocktransform")', // Define condition to show or hide menu entry. Use 'isModEnabled("mymodule")' if entry must be visible if module is enabled.
	...
	public function init($options = '')
	{
		...
		$result = $this->_load_tables('/StockTransform/sql/');
		if ($result < 0) {
		...
		// Document templates
		$moduledir = dol_sanitizeFileName('stocktransform');
		$myTmpObjects = array();
		$myTmpObjects['StockTransform'] = array('includerefgeneration'=>0, 'includedocgeneration'=>0);
